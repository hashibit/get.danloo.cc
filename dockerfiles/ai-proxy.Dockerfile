...
# Install ai-proxy project in editable mode
RUN uv pip install -e ./ai-proxy
...
COPY --from=builder /workspace/ai-proxy ./ai-proxy/
...
# Set working directory to ai-proxy service
WORKDIR /workspace/ai-proxy
...
CMD if [ -f entrypoint.sh ]; then ./entrypoint.sh; else python -m uvicorn main:app --host 0.0.0.0 --port 8091; fi
